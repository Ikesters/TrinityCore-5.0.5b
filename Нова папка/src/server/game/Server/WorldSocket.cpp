...
int WorldSocket::SendPacket(WorldPacket const& pct)
{
    ...
    if (m_Session)
         sLog->outError(LOG_FILTER_OPCODES, "S->C: %s %s", m_Session->GetPlayerInfo().c_str(), GetOpcodeNameForLogging(pkt->GetOpcode()).c_str());
...
int WorldSocket::ProcessIncoming(WorldPacket* new_pct)
{
    ...
    if (m_Session)
         sLog->outError(LOG_FILTER_OPCODES, "C->S: %s %s", m_Session->GetPlayerInfo().c_str(), opcodeName.c_str());
...
//        PreparedStatement* stmt = LoginDatabase.GetPreparedStatement(LOGIN_UPD_MUTE_TIME_LOGIN);
    ...
    AccountTypes allowedAccountType = sWorld->GetPlayerSecurityLimit();
     sLog->outError(LOG_FILTER_NETWORKIO, "Allowed Level: %u Player Level %u", allowedAccountType, AccountTypes(security));
