...
int Master::Run()
{
    ...
    sLog->outInfo(LOG_FILTER_WORLDSERVER, "HellfireEmu :: WorldServer Loading...");
    ...
    #ifdef _WIN32
    {
        ...
        if (Prio)
        {
            if (SetPriorityClass(hProcess, HIGH_PRIORITY_CLASS))
                sLog->outInfo(LOG_FILTER_WORLDSERVER, "HellfireEmu WorldServer process priority class set to HIGH");
            else
                sLog->outError(LOG_FILTER_WORLDSERVER, "Can't set HellfireEmu WorldServer process priority class.");
        }
    }
    #endif
    ...
    sLog->outInfo(LOG_FILTER_WORLDSERVER, "%s Worldserver is now Connectable!", _FULLVERSION);
...
bool Master::_StartDB()
{
    ...
    sLog->outInfo(LOG_FILTER_WORLDSERVER, "World Database version is up to date: %s", sWorld->GetDBVersion());
    return true;
}
