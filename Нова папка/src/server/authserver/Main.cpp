...
# define _TRINITY_REALM_CONFIG  "auth.conf"
#endif
...
extern int main(int argc, char **argv)
{
	...
	sLog->outInfo(LOG_FILTER_AUTHSERVER, "HellfireEmu :: LoginServer");
    sLog->outInfo(LOG_FILTER_AUTHSERVER, "%s (LoginServer)", _FULLVERSION);
    ...
    {
        sLog->outError(LOG_FILTER_AUTHSERVER, "Login server can not bind to %s:%d", bind_ip.c_str(), rmport);
        return 1;
    }
...
#ifdef _WIN32
    {
        ...
        if (Prio)
        {
            if (SetPriorityClass(hProcess, HIGH_PRIORITY_CLASS))
                sLog->outInfo(LOG_FILTER_AUTHSERVER, "The login server process priority class has been set to HIGH");
            else
                sLog->outError(LOG_FILTER_AUTHSERVER, "Can't set login server process priority class.");
...
bool StartDB()
{
    ...
    sLog->outInfo(LOG_FILTER_AUTHSERVER, "Started login database connection pool.");
